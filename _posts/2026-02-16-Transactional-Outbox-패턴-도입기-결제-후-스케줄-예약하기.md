...
Dual Write란 하나의 비즈니스 작업이 서로 다른 여러 시스템(DB, 외부 API, 메시지 브로커 등)에 각각 쓰기 작업을 수행하는 구조를 말한다. 두 시스템으로의 쓰기 작업은 하나의 원자적 연산으로 보장되지 않기 때문에, 일부만 반영된 상태가 발생할 수 있다.
...
## ✅ 정리
- 결제와 스케줄 예약은 하나의 트랜잭션으로 묶을 수 없는 분산 환경에서의 문제였다.
