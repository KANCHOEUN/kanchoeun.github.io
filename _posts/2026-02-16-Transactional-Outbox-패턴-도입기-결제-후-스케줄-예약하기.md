...
이 둘을 구분할 수 없다면 재시도는 시작조차 할 수 없다. 재시도를 위해서는 수행해야 할 작업이 어딘가엔 기록되어 있어야 했다.
이를 위해 결제 트랜잭션에서 Payment와 Subscription의 상태를 변경할 때 스케줄 예약 요청도 함께 기록하기로 하였다.
...
#### Dual Write Problem
이러한 문제는 일반적으로 Dual Write Problem이라고 한다.

Dual Write란 하나의 비즈니스 작업이 서로 다른 여러 시스템(DB, 외부 API, 메시지 브로커 등)에 각각 쓰기 작업을 수행하는 구조를 말한다. 두 시스템의 쓰기 작업은 하나의 원자적 연산으로 보장되지 않기 때문에, 일부만 반영된 상태가 발생할 수 있다.

- DB의 상태는 변경에 성공하여 커밋되었지만, 메시지가 유실된 경우
- DB의 상태 변경에 실패하여 롤백되었지만, 이미 이벤트가 소비된 경우

결제는 확정되었지만 내부 트랜잭션은 롤백되거나, 반대로 내부 상태는 변경되었지만 외부 스케줄 예약은 실패하는 상황이 여기에 해당한다.
...
외부 호출을 트랜잭션에 포함시키는 대신, 외부로 전달해야 할 작업을 DB에 기록하고 이후 별도의 프로세스가 이를 처리하도록 분리한다. 이렇게 하면 DB 상태 변경과 작업 기록은 하나의 트랜잭션으로 묶여 원자적으로 처리되고, 외부 호출의 실패가 곧바로 데이터 정합성 문제로 이어지지 않게 된다.

<br/>

### 🤨 불필요한 조회가 많은 것 같은데, 다른 방법은 없었을까?

Polling 기반 Outbox는 단순하지만, 주기적으로 테이블을 조회해야 한다는 점에서 비효율적으로 보였다. 그래서 Polling 없이 변경 사항을 전달하는 방법도 검토해보았다.

#### (1) Message Broker 기반 Outbox
Outbox 테이블의 변경 사항을 Kafka 같은 메시지 브로커로 보내는 구조를 만들 수도 있다.

#### (2) CDC; Change Data Capture 기반 Outbox
Debezium와 같은 CDC 툴을 사용하는 방법도 있다.

#### (3) SAGA 패턴
이외에도 보상 트랜잭션을 통해 작업을 되돌리는 SAGA 패턴의 경우도 고려해볼 수 있다.

ex) 결제 성공 → 스케줄 예약 실패 → 환불(보상) 트랜잭션 실행
